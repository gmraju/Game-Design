...
public class ProjectileThrower : MonoBehaviour {
    ...
    private void ResetForFiring()
    {
        ...
        if (SceneManager.GetActiveScene().name == "Robot")
        {
            RobotTalk.collisionCount = 0;
            RobotTalk.firstHit = true;
        }
    }
        ...
        switch (firingState)
        {
            case FiringState.Idle:
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    if (SceneManager.GetActiveScene().name == "Robot")
                        GameObject.Find("Instructions").GetComponent<InstructScript>().killSelf();
                }
                break;
